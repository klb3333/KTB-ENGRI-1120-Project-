...
html"""
<p style="font-size:20px;">KTB: Kenneth Burnett, Temilola Omojola, Bria Reed </br>
Smith School of Chemical and Biomolecular Engineering, Cornell University, Ithaca NY 14850</p>
"""

# ╔═╡ 4a9766d4-97cb-4798-9c51-2bfc68f1ed67
md"""
### Introduction
Hello, for our project we produced hydrazine from xylose. We produced hydrazine using a parallel approach and found how to produce over 95% purity at 1 gram/hr. The process in which our team produced hydrazine is detailed throughout the project. Hydrazine, according to the National Library of Medicine has uses in chemical blowing agents, textile dyes, and rocket fuel, among many other uses. 
"""
...
First, we looked at the data from the “Model_v2.vff” file to see what was needed to be supplied to the reaction to make hydrazine with xylose. 

Next, the reactants that were needed to make the reaction run according to the “Model_v2.vff” were put into Julia. Following the “Model_v2.vff” file, the reaction needed ammonia, nitric oxide, and NADH, but after trying to run the reaction in Julia, the reactants needed to be changed. Since NADH was produced in the reaction, NADH did not need to be supplied, however, oxygen and xylose needed to be supplied in addition to ammonia and nitric oxide. Xylose needed to be supplied because it was the sugar that was assigned to the group.

The products needed to be optimized, which allowed us to put enough reactants in to make 1.0 gram of hydrazine per hour at a 95% purity rate. However, through flux balance analysis, it was determined that only 2.466 millimoles of hydrazine per hour was able to be run through one of the reactors, and that determined how much of each compound to supply in the input stream. 

To get a purity rate of 95%, the amount of levels of separation needed to be optimized as well. To get this purity percent we needed 5 levels of separation. Producing 2.466 millimoles of hydrazine per hour helped to optimize this rate. To get to a minimum of 5 levels, the amount of reactants that were put into this reaction were reduced so that there was not an excess amount of reactants. Since we needed 5 levels of separation, four separators were required. 

After optimizing the reaction, the amount of chips needed to make the reaction run was optimized. When the amount of chips were optimized, we needed 39 chips to run this reaction. 

Next, the amount of reactants that needed to be put in the reactors to produce approximately 1.0 gram of hydrazine per hour at a 95% purity rate were priced, so that these reactants can be bought in bulk. We priced the reactants for the factory working hours. The factory will run on all working days of the year (261) for 24 hours a day. 

After pricing the amount of reactants that will be bought in bulk, the financial analysis was created in a pluto notebook using NPV. NPV is used for the economic feasibility of this project. NPV also helps to calculate if your group would be better off investing in stock rather than investing in this project because of the fact that your project may not be successful and because money depreciates over time. 

Lastly, the final results of this project were coded into a pluto notebook in LaTeX code. 

The values of the different conversions that were carried out are shown below:

For a single chip:

Oxygen= 0.411 mmol/hr

Ammonia= 2.466 mmol/hr

Nitric oxide= 1.233 mmol/hr

d-xylose= 2.466 mmol/hr

Converting from mmol/hr to g/hr then multiplying by 6264 hours gives:

Oxygen= 3.126988 g

Ammonia= 35.42292 g

D-xylose= 2.00567016 g 

Nitric oxide= 20.0811312 g

To get the volume of ammonia and nitric oxide required, we divide the mass by the density to get:

Density of ammonia= 0.73 kg/m^3

Density of nitric oxide= 1.34 kg/m^3

Volume of ammonia required= 49ml

Volume of nitric oxide required= 15ml

After determining how much of each component is needed to run the reaction continuously for 261 days, we went out to Sigma Aldrich to estimate the costs of each one and purchased all our products from there.

The prices from Sigma Aldrich:
1. Ammonia= $ 634.00 for 170 grams
2. Nitric oxide= $ 721.00 for 17 liters
3. d-xylose= 1 kg for $ 69.70

For the physical equipment:

1. Reactors= $ 3900

2. For the separators= $ 80 (4 for $ 20 each)
...
Costs of equipment needed= $ 4980

Total estimated cost of project= $ 1424.7 + $ 4980= $ 6404.7
...
# ╔═╡ 36465f8e-5d17-4600-b77e-bbcd67a395bf
number_of_chips = 39
...
# ╔═╡ 55c1db1d-6903-4b58-9db0-e28fe6d13ac3
md"""
### Financial Balance
We now analyze the financial performance by determining the economic feasibility of the project. To do this, we compare it to an alternative investment yielding a miniumum if 1% per year and a maximum of 10%. The following calculations show the determination of the net present value from these 2 discount rates.

In order to calculate the NPV values, we had to compute the net cash flow values for each year as we decided to use a period of 4 years for our economic feasibility study.

For year 1: The net cash flow value was calculated by using the formula:

Cash product (revenue) - Cost of materials - cost of equipment.

Cash product= Amount of product made in a year multiplied by the total costs of the project + Amount made from selling extra d-xylose.

Amount of product made: 6336.28656 g

Amount made from selling excess d-xylose= We decided to sell 39 packs of 25 g of excess d-xylose each for $ 66.00= $ 2574

Cash product= $ 40,584,588.15

Net Cash Flow for year 1: $ 40,584,588.15 - $ 6404.7 = $ 40,578,183

For the three subsequent years: For these years, we only subtract the cost of materials because we wouldn’t need to purchase extra equipment.

The net cash flow=Cash product - cost of materials

The net cash flow= $40,584,588.15 - $ 1424.7= $ 40,583,163.45


Calculation for NPV of Alternative investment Yielding Minimum of 1% per Year:
"""


# ╔═╡ ff54c73f-c003-4061-a535-a33a45bbdb06
# lifetime = 3 future years (period of time to payback the investment)
T = 3

# ╔═╡ e0401985-ffe5-4249-8988-ac32951c9d07
# discount rate: risk-free interest rate 
# assume constant over the payback lifetime -
discount_rate_1 = 0.01

# ╔═╡ eeff2e96-1a83-4116-a951-f4b6ef43ad63
# Setup the cash flow array -
# we pay in year 1: $230K (out, -ve)
# we save in year(s) 2,3 and 4: 90K (in,+ve)
CF_array_1 = [40578183.45, 40583163.45, 40583163.45, 40583163.45]

# ╔═╡ 32e456f8-db82-40aa-b45a-06feed5163c7
# compute the discount terms -
begin
	
	# main -
	discount_array_1 = ones(T+1)
	for time_index_1 = 2:(T+1)
    	tmp_term_1 = (1+discount_rate_1)^(time_index_1-1)
    	discount_array_1[time_index_1] = tmp_term_1
	end
...
# ╔═╡ e4738494-f383-4fad-8d58-c5308f594474
# what is the PV?
PV_1 = CF_array_1.*(1.0./discount_array_1)

# ╔═╡ 759e3046-9d68-4b1b-b3c0-0780f9d555c9
# what is the NPV?
NPV_1 = sum(PV_1)

# ╔═╡ 74041534-957f-4003-b0d9-2d379b53afd3
md"""
Calculation for NPV of Alternative investment Yielding Minimum of 10% per Year:
"""

# ╔═╡ b038c3e5-b07d-4fc7-9c1d-d171e0a4b070
# lifetime = 3 future years (period of time to payback the investment)
Time = 3

# ╔═╡ e226b95b-6b14-4392-bb2d-2c864ca9c8cc
# discount rate: risk-free interest rate 
# assume constant over the payback lifetime -
discount_rate_2 = 0.1

# ╔═╡ f317d3a2-0ba6-49e1-a779-7e6a877ac358
# Setup the cash flow array -
# we pay in year 1: $230K (out, -ve)
# we save in year(s) 2,3 and 4: 90K (in,+ve)
CF_array_2 = [40578183.45, 40583163.45, 40583163.45, 40583163.45]

# ╔═╡ fdcff98a-943f-4d07-bb68-b6a3ab6ed3b4
# compute the discount terms -
begin
	
	# main -
	discount_array_2 = ones(Time+1)
	for time_index_2 = 2:(Time+1)
    	tmp_term_2 = (1+discount_rate_2)^(time_index_2-1)
    	discount_array_2[time_index_2] = tmp_term_2
	end
...
# ╔═╡ e776edc3-4bd7-4c23-a996-3cbf4b31a468
# what is the PV?
PV_2 = CF_array_2.*(1.0./discount_array_2)

# ╔═╡ 671facf5-c1a2-4db0-ba0e-a693733a7f14
# what is the NPV?
NPV_2 = sum(PV_2)
...
The requirement that the client had for this project was a mass flow rate of 1 g/hr as well as 95% purity of the hydrazine that we produced. We were able to supply all input components at the most optimal flow rate to get this target of 1 g/hr and we needed only 4 points of separations to purify the product. Therefore,our production process up until the points of separation minimized the costs of this project as much as we could given the restrictions of a maximum of 2.466 mmol/hr of hydrazine to be produced from each reactor.

However, we also produced methylglyoxal as a by-product,and this was not used so it went to waste. The costs could have further minimized the cost of this project by purifying and selling this by-product to offset the costs of production.

In addition, we actually offset part of the costs of the project by selling excess d-xylose that weren't needed for our project. This added to our revenue generated, thereby offsetting costs.

To further analyze the economic feasibility of this project,we calculated the NPV values for an alternative investment yielding 1% per year and another one yielding 10% per year. Our calculations show that for an alternative investment yielding 1% per year, our NPV is 1.41503 and for the alternative investment yielding 10% per year, our NPV would be 1.5994. These two positive values indicate that our project will generate more income than the alternative investment. This alternative investment would be the investment of the cost of our project, $ 6404.7 growing risk free at these two discount rates.

Therefore, for an alternative investment yielding 1%, our project generates more income and similarly, for an alternative investment yielding 10%,the project also generates more income.
...
md"""
### Conclusions
The project was successful in that we were able to deliver our product to fit the specifications of a 95% purity rate at 1 g/hr that the clients required.The project also proved to pass our economic feasibility study as it showed that we have a positive net present value at a discount rate of 1% and 10% for an alternative investment. However, we also acknowledge that we could have been more resourceful by taking advantage of the methylglyoxal produced and selling it to minimize costs and generate revenue.We could have also explored alternative setups to our project such as setting up the chips in series, that might have been cheaper and perhaps required less materials than the parallel set up.
...
Ammonia link: https://www.sigmaaldrich.com/US/en/product/aldrich/294993 

Nitric Oxide link: https://www.sigmaaldrich.com/US/en/product/aldrich/295566

Xylose link: https://www.sigmaaldrich.com/US/en/product/usp/1722005
